import { Card } from "../components/Card";

type NaturalPeriodRow = {
  label: string;
  rc: string;
  s: string;
  w: string;
};

const NATURAL_PERIOD_TABLE: NaturalPeriodRow[] = [
  { label: "平屋", rc: "-", s: "0.10~0.18", w: "0.1~0.2" },
  { label: "2階建", rc: "0.15~0.20", s: "0.18~0.25", w: "0.2~0.3" },
  { label: "5階建", rc: "0.25~0.35", s: "0.35~0.50", w: "-" },
  { label: "10階建", rc: "0.6~1.0", s: "0.9~1.5", w: "-" },
  { label: "20階建", rc: "0.9~1.2", s: "1.8~3.0", w: "-" },
];

const INTENSITY_TABLE = [
  { level: 7, name: "激震", range: "400 〜" },
  { level: 6, name: "烈震", range: "250 〜 400" },
  { level: 5, name: "強震", range: "80 〜 250" },
  { level: 4, name: "中震", range: "25 〜 80" },
  { level: 3, name: "弱震", range: "8 〜 25" },
  { level: 2, name: "軽震", range: "2.5 〜 8" },
  { level: 1, name: "微震", range: "0.8 〜 2.5" },
  { level: 0, name: "無震", range: "〜 0.8" },
];

const INTENSITY_COLOR: Record<number, string> = {
  7: "bg-red-700",
  6: "bg-red-500",
  5: "bg-orange-500",
  4: "bg-yellow-500",
  3: "bg-blue-600",
  2: "bg-blue-400",
  1: "bg-gray-500",
  0: "bg-gray-400",
};

function AboutPage() {
  return (
    <div className="p-4 sm:p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        <Card className="p-6 sm:p-8">
          <h1 className="text-h1 text-neutral-900 mb-6">
            このアプリについて
          </h1>

          <section className="mb-8">
            <h2 className="text-h3 text-neutral-700 mb-3 flex items-center gap-2">
              <span className="w-1 h-5 bg-primary rounded-full inline-block" />
              取扱い説明書
            </h2>
            <ul className="space-y-2.5 text-sm text-neutral-600 leading-relaxed">
              <li className="flex gap-2">
                <span className="text-primary-light mt-0.5">・</span>
                任意地点の緯度と経度を入力すると、主な既往地震に対する推定応答値がわかります。
              </li>
              <li className="flex gap-2">
                <span className="text-primary-light mt-0.5">・</span>
                対象構造物の地盤種別と固有周期を絞り込めると、より正確な推定応答値がわかります。
              </li>
              <li className="flex gap-2">
                <span className="text-primary-light mt-0.5">・</span>
                旧震度階級（震度0〜7）と推定応答値（応答加速度）の関係は下表の通りです。なお、980
                gal が 1G（重力加速度）です。
              </li>
            </ul>
          </section>

          <section className="mb-8">
            <h2 className="text-h3 text-neutral-700 mb-3 flex items-center gap-2">
              <span className="w-1 h-5 bg-primary rounded-full inline-block" />
              旧震度階級と推定応答値の対応
            </h2>
            <div className="overflow-x-auto rounded-lg border border-line">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-neutral-50 text-neutral-500 text-xs">
                    <th className="px-4 py-3 text-left font-medium">震度階級</th>
                    <th className="px-4 py-3 text-left font-medium">名称</th>
                    <th className="px-4 py-3 text-right font-medium">
                      応答加速度 (gal)
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100">
                  {INTENSITY_TABLE.map((row) => (
                    <tr
                      key={row.level}
                      className="transition-colors duration-100 ease-standard hover:bg-neutral-50"
                    >
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block font-bold px-2 py-0.5 rounded text-white text-xs tabular-nums ${INTENSITY_COLOR[row.level]}`}
                        >
                          震度 {row.level}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-neutral-700">
                        {row.name}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums text-neutral-600">
                        {row.range}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </section>

          <section className="mb-8">
            <h2 className="text-h3 text-neutral-700 mb-3 flex items-center gap-2">
              <span className="w-1 h-5 bg-primary rounded-full inline-block" />
              地盤種別
            </h2>
            <div className="space-y-3">
              {[
                {
                  label: "第Ⅰ種（硬質地盤）",
                  desc: "岩盤や砂礫層。最も揺れにくい。",
                },
                {
                  label: "第Ⅱ種（普通地盤）",
                  desc: "洪積層など、比較的しっかりした土砂層。",
                },
                {
                  label: "第Ⅲ種（軟弱地盤）",
                  desc: "沖積層、埋立地、粘性土など。地震の際に最も揺れやすい。",
                },
              ].map((item) => (
                <div
                  key={item.label}
                  className="flex gap-3 items-start border border-line rounded-lg px-4 py-3 text-sm bg-surface-subtle text-neutral-700"
                >
                  <span className="font-semibold whitespace-nowrap">
                    {item.label}
                  </span>
                  <span className="text-neutral-600">{item.desc}</span>
                </div>
              ))}
            </div>
          </section>

          <section className="mb-8">
            <h2 className="text-h3 text-neutral-700 mb-1 flex items-center gap-2">
              <span className="w-1 h-5 bg-primary rounded-full inline-block" />
              固有周期の目安 (s)
            </h2>
            <p className="text-caption text-neutral-400 mb-3">
              建物の構造形式・階数による固有周期のおおよその目安です。
            </p>
            <div className="overflow-x-auto rounded-lg border border-line">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-neutral-50 text-neutral-500 text-xs">
                    <th className="px-4 py-3 text-left"></th>
                    <th className="px-4 py-3 text-center font-medium" colSpan={2}>
                      鉄筋コンクリート
                    </th>
                    <th
                      className="px-4 py-3 text-center font-medium border-l border-neutral-200"
                      colSpan={2}
                    >
                      鉄骨
                    </th>
                    <th
                      className="px-4 py-3 text-center font-medium border-l border-neutral-200"
                      colSpan={2}
                    >
                      木造
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100">
                  <tr className="bg-neutral-50 text-xs">
                    <td className="px-4 py-2 font-medium text-neutral-600">
                      築年数
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600">
                      <div className="text-neutral-400 font-normal text-xs">
                        1996年以降
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600">
                      <div className="text-neutral-400 font-normal text-xs">
                        1995年以前
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600 border-l border-neutral-100">
                      <div className="text-neutral-400 font-normal text-xs">
                        1996年以降
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600">
                      <div className="text-neutral-400 font-normal text-xs">
                        1995年以前
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600 border-l border-neutral-100">
                      <div className="text-neutral-400 font-normal text-xs">
                        1996年以降
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center tabular-nums text-neutral-600">
                      <div className="text-neutral-400 font-normal text-xs">
                        1995年以前
                      </div>
                    </td>
                  </tr>
                  {NATURAL_PERIOD_TABLE.map((row) => (
                    <tr
                      key={row.label}
                      className="transition-colors duration-100 ease-standard hover:bg-neutral-50"
                    >
                      <td className="px-4 py-3 font-medium text-neutral-700">
                        {row.label}
                      </td>
                      <td
                        className="px-4 py-3 text-center tabular-nums text-neutral-600"
                        colSpan={2}
                      >
                        {row.rc}
                      </td>
                      <td
                        className="px-4 py-3 text-center tabular-nums text-neutral-600 border-l border-neutral-100"
                        colSpan={2}
                      >
                        {row.s}
                      </td>
                      <td
                        className="px-4 py-3 text-center tabular-nums text-neutral-600 border-l border-neutral-100"
                        colSpan={2}
                      >
                        {row.w}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </section>

          <section>
            <h2 className="text-h3 text-neutral-700 mb-3 flex items-center gap-2">
              <span className="w-1 h-5 bg-orange-400 rounded-full inline-block" />
              注意事項
            </h2>
            <ul className="space-y-2.5 text-sm text-neutral-600 leading-relaxed">
              <li className="flex gap-2">
                <span className="text-orange-400 mt-0.5">・</span>
                本アプリで使用する震度は、2026年3月現在の震度階級ではなく
                <strong className="text-neutral-700">旧震度階級（0〜7）</strong>
                です。
              </li>
              <li className="flex gap-2">
                <span className="text-orange-400 mt-0.5">・</span>
                掲載している地震は主要なものに絞っており、抜け漏れが存在する場合があります。また、一部の記録には解釈が含まれます。なお、今後発生する地震が随時追加されるとは限りません。
              </li>
              <li className="flex gap-2">
                <span className="text-orange-400 mt-0.5">・</span>
                本アプリの
                <strong className="text-neutral-700">
                  営利目的での利用を禁止
                </strong>
                します。
              </li>
            </ul>
          </section>
        </Card>
      </div>
    </div>
  );
}

export default AboutPage;
